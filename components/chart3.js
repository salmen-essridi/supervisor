...
const ChartWithZoom = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetch('/api/get-trace-data-multi?tp='+ targetPeriod )
...
const onBtnExport = () => {
  ...
  return (
    <div className="ChartWithZoom">
      ...
      <div style={{ textAlign: 'center', fontSize: '30px', margin: '50px' }}> Période : {periode} <br />  utilisateurs : {files.length}, produits visités : {data.length}</div>
      ...
      <div className="ag-theme-alpine" style={{ height: "400px", padding: '50px' }}>
        ...
        onGridReady={onGridReady}
          ...
          <AgGridColumn field="user"  filter={true} sortable={true} ></AgGridColumn>
